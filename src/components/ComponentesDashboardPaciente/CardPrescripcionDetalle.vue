<template>
  <v-card elevation="3" class="card-home-prescripcion">
    ...
    <div v-for="item in listaDiagnosticos" :key="item.codigo_enfermedad">
      ...
      <h4 class="text-blue">Prescripción</h4>
      <div
        v-for="itemPrescripcion in item.prescripcion"
        ...
        <v-list-item>
          <v-list-item-content>
            <v-list-item-title class="item-prescripcion"
              ...
              {{ itemPrescripcion.concentracion }} ({{
                itemPrescripcion.formula
              }})</v-list-item-title
            >
            <v-list-item-subtitle class="item-prescripcion-subtitle"
              >Durante {{ itemPrescripcion.dosis.tiempo.valor }}
              {{ itemPrescripcion.dosis.tiempo.medida }} cada
              {{ itemPrescripcion.dosis.frecuencia.valor }}
              {{
                itemPrescripcion.dosis.frecuencia.medida
