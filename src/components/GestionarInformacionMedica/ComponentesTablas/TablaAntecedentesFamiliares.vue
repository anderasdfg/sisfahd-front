  ...
  >
    <template v-slot:top>
      <v-toolbar
        flat
      >
        <v-toolbar-title>Lista de Antecedentes Familiares</v-toolbar-title>
        <v-divider
          class="mx-4"
          inset
          vertical
...
export default { 
  name:'TablaAntecedentesFamiliares',
  components:{
    TablaObservaciones
  },
  props:["lista_familiares"],
  data(){
    return{
      lista_observaciones:[],
      editedItem: {
  ...
  computed:{
    formTitle () {
      return this.editedIndex === -1 ? 'Registre un antecedente familiar' : 'Edite un antecedente familiar'
    },
  },
  methods:{
    ...
    deleteItem (item) {
      this.editedIndex = this.lista_familiares.indexOf(item)
    ...
    deleteItemConfirm () {
      this.lista_familiares.splice(this.editedIndex, 1)
      this.$emit('emited_lista_tutores_legales',this.lista_familiares)
      this.closeDelete()
    },
    close(){
      this.dialog = false
    ...
    save() {
      ...
      if (this.editedIndex > -1) {
        Object.assign(this.lista_familiares[this.editedIndex], this.editedItem)       
      } else {
        this.lista_familiares.push(this.editedItem)
      }
      this.close();
    },
  }
